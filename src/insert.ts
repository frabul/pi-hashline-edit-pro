...
export function assertInsertReq(request: unknown): asserts request is InsertReq {
  ...
  if (typeof request.anchor !== "string" || request.anchor.length === 0) {
    throw new Error('[E_BAD_SHAPE] Insert request requires an "anchor" string (3-char hash from read_with_anchors output).');
  }
...
const insertToolSchema = Type.Object(
  {
    ...
    anchor: Type.String({
      description:
        'Bare 3-char HASH only (e.g. "aB3"): copy just the hash from the leftmost column of a read_with_anchors row like `aB3│content`; never the line content. A pasted diff row like `+aB3│x` or a `HASH│` prefix is stripped automatically with a warning. The anchored line is preserved; the new lines go after or before it.',
    }),
